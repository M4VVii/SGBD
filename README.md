...
Antes de comenzar, explicare los términos que se usarán en todo el manual.

**Puerto:** Una computadora tiene una sola dirección de red (IP), pero puede correr muchos servicios a la vez. El **puerto** es un número de 0 a 65535 que funciona como el "número de departamento" dentro de ese edificio, identifica a qué programa concreto va dirigida una conexión. PostgreSQL usa por convención el **puerto 5432**. Un puerto solo puede estar ocupado por un programa a la vez, hecho que será decisivo en la sección 3.

**Proceso y servicio (demonio):** Un **proceso** es un programa en ejecución. Un **servicio** o **demonio** (*daemon*) es un proceso que corre en segundo plano, sin ventana ni interacción directa, normalmente arrancado por el sistema operativo al encender el equipo. El servidor PostgreSQL es un demonio.
...
**Virtualización:** Técnica que permite ejecutar un sistema aislado dentro de otro. Existen dos grandes familias, y distinguirlas es el corazón de este manual.
...
#### 1.5.2 Máquina virtual (para contrastar)
...
**[Insertar captura de pantalla de: la pantalla de información del sistema de Linux Mint (menú → Preferencias → Detalles del sistema o el comando `neofetch`), mostrando la versión de Mint y el entorno Cinnamon]**
...
- **Registro (*registry*).** Un repositorio en línea de imágenes. El público por defecto es **Docker Hub**.
...
**Finalidad conceptual:** esta llave permite que APT verifique que los paquetes que descargará fueron firmados realmente por Docker Inc. y no fueron alterados en tránsito.

#### Paso 3: Añadir el repositorio de Docker (con la corrección para Linux Mint)
...
| `$(. /etc/os-release && echo "$UBUNTU_CODENAME")` | **Esta es la corrección clave para Mint.** El punto (`.`) carga el archivo `/etc/os-release` como si fueran variables de la terminal; de ahí se extrae `UBUNTU_CODENAME`, que contiene el nombre clave de la versión de *Ubuntu* en la que se basa Mint (por ejemplo `noble`), en vez del nombre clave de Mint. Si se usara `lsb_release -cs` —como indican los tutoriales de Ubuntu— se obtendría `wilma` o `xia`, y el repositorio devolvería un error 404. |
...
**[Insertar captura de pantalla de: la terminal tras ejecutar `sudo apt update`, mostrando la línea donde aparece `download.docker.com` entre los orígenes consultados]**
...
> ### DATO...
> Pertenecer al grupo `docker` equivale, en la práctica, a tener privilegios de administrador, porque un contenedor puede montar el disco del anfitrión. Es cómodo en un equipo personal de desarrollo, pero en un servidor de producción se considera un riesgo de seguridad.

**[Insertar captura de pantalla de: la salida del comando `sudo docker run --rm hello-world` con el mensaje "Hello from Docker!"]**

### 2.3 El archivo `Dockerfile`

#### 2.3.1 ¿Qué es?

Un **`Dockerfile`** es un archivo de texto plano, sin extensión, que contiene la **receta para construir una imagen**. Cada línea es una instrucción que genera una capa nueva sobre la anterior. Se lee de arriba abajo y su resultado es determinista: el mismo `Dockerfile` produce la misma imagen en cualquier equipo.

> ### DATO...
> Para PostgreSQL **no siempre es necesario escribir un `Dockerfile`**: la imagen oficial `postgres` ya está construida y lista. Se crea un `Dockerfile` propio solo cuando se quiere *personalizar* esa imagen base: agregar extensiones, cambiar la zona horaria o incluir un guion SQL de inicialización. Ese es exactamente el caso que se documenta a continuación, con fines didácticos.

#### 2.3.2 Estructura de carpetas del proyecto
...
LABEL description="PostgreSQL 17 personalizado para la práctica de SGBD"
...
Contenido de ejemplo para `init/01-esquema.sql`:
...
INSERT INTO alumnos (matricula, nombre, carrera) VALUES
    ('A00123456', 'Ana Gómez',  'Ingeniería en Sistemas'),
    ('A00123457', 'Luis Pérez', 'Ingeniería en Software');
```
...
**Docker Compose** es la herramienta de orquestación para un solo equipo. Su archivo `docker-compose.yml` es **declarativo**: en lugar de describir los pasos a seguir, se describe el **estado final deseado**, y Compose se encarga de alcanzarlo.
...
services:
  ...
  base_datos:
    ...
    image: postgres-escuela:1.0
    container_name: pg_escuela
  ...
  adminer:
    ...
    container_name: adminer_escuela
    restart: unless-stopped
    depends_on:
      base_datos:
...
**`image: postgres-escuela:1.0`** — Nombre y versión que se le dará a la imagen resultante. (Si se omitiera `build:`, esta línea indicaría en cambio qué imagen *descargar* de Docker Hub.)

**`container_name: pg_escuela`** — Nombre fijo y legible del contenedor. Sin esta línea, Docker generaría uno automático como `proyecto-postgres-base_datos-1`.
...
**`ports:`** — El **mapeo de puertos**. Esta es la línea que conecta el mundo aislado del contenedor con la computadora.
  ...
  - **Advertencia crítica:** si ya existe una instalación nativa de PostgreSQL ocupando el 5432 (sección 3), este contenedor fallará con `port is already allocated`. La solución es poner `PUERTO_HOST=5433` en el archivo `.env`, con lo que el mapeo queda `"5433:5432"`.
...
**`volumes:` (bloque final, sin sangría)** — Declara formalmente los volúmenes nombrados que usan los servicios. `name: volumen_datos_escuela` fija un nombre fijo en lugar del generado automáticamente.
...
**[Insertar captura de pantalla de: la terminal ejecutando `docker compose up -d`, con las líneas de descarga de la imagen y los mensajes `Created` / `Started` de ambos contenedores]**
...
| Elemento | Significado |
|---|---|
| `logs` | Muestra la salida estándar del contenedor: aquí, el diario de arranque de PostgreSQL. |
...
**[Insertar captura de pantalla de: la salida de `docker compose ps` mostrando ambos contenedores en estado `Up` y `healthy`]**
...
| `pg_escuela` | El `container_name` definido en el YAML. |
| `psql` | El cliente oficial de PostgreSQL en línea de comandos, ya incluido en la imagen. |
| `-U admin_escuela` | *User*: usuario con el que se inicia sesión. |
| `-d escuela_db` | *Database*: base de datos a la que se conecta. |
...
**[Insertar captura de pantalla de: la sesión de `psql` dentro del contenedor mostrando el resultado de `SELECT version();` y las dos filas de la tabla `alumnos`]**
...
| `docker volume ls` | Lista los volúmenes existentes, para comprobar que `volumen_datos_escuela` sigue ahí. |
...
### 3.3 Paso 2: Consultar la versión disponible (opcional pero recomendable)
...
**[Insertar captura de pantalla de: la terminal durante la instalación con APT, mostrando el porcentaje de descarga y el mensaje final de configuración del paquete]**
...
**[Insertar captura de pantalla de: la salida completa de `systemctl status postgresql@17-main` mostrando `active (running)` en verde]**
...
```sql
ALTER USER postgres WITH PASSWORD 'MiContrasenaSegura2026';

CREATE DATABASE escuela_nativa;

CREATE USER admin_escuela WITH PASSWORD 'OtraClaveSegura2026';
GRANT ALL PRIVILEGES ON DATABASE escuela_nativa TO admin_escuela;
...
### 3.7 Paso 6: Permitir conexiones con contraseña (configuración)
...
|---|---|
| `listen_addresses = 'localhost'` | Direcciones de red en las que el servidor escucha. `localhost` (valor por defecto) basta para conectarse desde el mismo equipo. Cambiarlo a `'*'` lo abriría a toda la red, lo que **no se recomienda** en un equipo de práctica. |
...
En el contexto de bases de datos, un cliente gráfico permite:
...
### 4.2 Opción A: Adminer (ya incluido en el `docker-compose.yml`)
...
| Usuario | `admin_escuela` | El definido en `POSTGRES_USER`. |
| Contraseña | La de `POSTGRES_PASSWORD` | |
| Base de datos | `escuela_db` | La definida en `POSTGRES_DB`. |

3. Pulsar *Entrar*. Debe aparecer el listado de tablas, incluida `alumnos`.

**[Insertar captura de pantalla de: el formulario de inicio de sesión de Adminer con los campos llenados]**

**[Insertar captura de pantalla de: la vista de Adminer mostrando la tabla `alumnos` con sus dos registros]**

### 4.3 Opción B: DBeaver Community

#### Paso 1: Instalación en Linux Mint

**Vía gestor de programas (la más sencilla):** abrir *Menú → Administración → Gestor de programas*, buscar "DBeaver" e instalar.

**Vía terminal con Flatpak** (preinstalado en Linux Mint):
...
1. Abrir DBeaver. Ir a **Base de datos → Nueva conexión** (o el icono del enchufe con el signo `+`).
2. Elegir **PostgreSQL** de la lista y pulsar *Siguiente*.
3. La primera vez, DBeaver ofrecerá **descargar el controlador JDBC**. Aceptar.
...
| **Base de datos** | `escuela_db` | `escuela_nativa` |
| **Usuario** | `admin_escuela` | `admin_escuela` o `postgres` |
| **Contraseña** | La del archivo `.env` | La fijada con `ALTER USER` |

Nota importante: aquí **sí** se escribe `localhost`, a diferencia de Adminer. DBeaver se ejecuta en Linux Mint (fuera de Docker), así que llega al contenedor a través del **mapeo de puertos** definido en `ports:`. Este contraste ilustra a la perfección para qué sirve el mapeo.

Conviene marcar la casilla **"Guardar contraseña"** para no tener que reescribirla.

**[Insertar captura de pantalla de: la ventana de configuración de conexión de DBeaver con los campos Host, Puerto, Base de datos y Usuario llenados]**
...
**[Insertar captura de pantalla de: el cuadro de diálogo de "Test Connection" exitoso, mostrando la versión del servidor PostgreSQL]**
...
La aparición de la fila en la cuadrícula de resultados constituye la **prueba definitiva** de que el servidor está instalado, activo, accesible por red y aceptando escrituras.

**[Insertar captura de pantalla de: el editor SQL de DBeaver con el script ejecutado y la cuadrícula de resultados mostrando la fila insertada]**

**[Insertar captura de pantalla de: el árbol de navegación izquierdo de DBeaver, expandido hasta Bases de datos → Esquemas → public → Tablas]**
...
La elección no es ideológica, sino contextual:
...
| # | Sección | Evidencia a capturar |
|---|---|---|
| 1 | 1.5 | Información del sistema de Linux Mint (versión y entorno Cinnamon) |
| 2 | 2.2 | `sudo apt update` mostrando `download.docker.com` entre los orígenes |
| 3 | 2.2 | `sudo docker run --rm hello-world` con el mensaje "Hello from Docker!" |
| 4 | 2.5 | `docker compose up -d` con los mensajes `Created` / `Started` |
| 5 | 2.6 | `docker compose ps` con ambos contenedores en `Up` y `healthy` |
| 6 | 2.6 | Sesión `psql` dentro del contenedor: `SELECT version();` y la tabla `alumnos` |
| 7 | 3.4 | Instalación con APT en progreso |
| 8 | 3.5 | `systemctl status postgresql@<versión>-main` en `active (running)` |
| 9 | 4.2 | Formulario de inicio de sesión de Adminer con los campos llenados |
| 10 | 4.2 | Adminer mostrando los registros de la tabla `alumnos` |
| 11 | 4.3 | Ventana de configuración de conexión de DBeaver |
| 12 | 4.3 | Diálogo de "Test Connection" exitoso con la versión del servidor |
| 13 | 4.3 | Editor SQL de DBeaver con la cuadrícula de resultados |
| 14 | 4.3 | Árbol de navegación de DBeaver expandido hasta las tablas |
